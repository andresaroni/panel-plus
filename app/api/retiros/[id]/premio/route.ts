...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const record = await prisma.retirar_saldo.findUnique({
    ...
    select: { premio_imagen: true, premio_mime: true },
  });
  if (!record?.premio_imagen) return new NextResponse(null, { status: 404 });

  const mime = record.premio_mime?.toLowerCase() ?? "";
  if (!allowedMime.has(mime)) return new NextResponse(null, { status: 415 });

  return new NextResponse(record.premio_imagen, {
    headers: {
      "Content-Type": mime,
      "Content-Disposition": `inline; filename="premio-${id}"`,
